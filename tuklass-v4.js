(function () {
    ...
    function renderModalGoogleButton() {
        ...
        const intent =
            window.tuklassAuthIntent === "login"
                ? "login"
                : "create";
    ...
    function openAuthModal(intent) {
        ...
        const description =
            document.getElementById("accountModalDescription");

        if (title) {
            title.textContent =
                window.tuklassAuthIntent === "login"
                    ? "Log in to Tuklass"
                    : "Create your Tuklass account";
        }

        if (description) {
            description.textContent =
                window.tuklassAuthIntent === "login"
                    ? "Continue with the Google account connected to your Tuklass account."
                    : "Continue with Google, then choose your username, school, and section.";
        }
    ...
    function closeAuthModal() {
        ...
        /* Use both the native hidden state and an inline display fallback.
           This makes closing reliable even if another stylesheet forces
           the modal layout with !important. */
        modal.hidden = true;
    ...
    window.closeCreateAccountModal = closeAuthModal;
    window.closeLoginModal = closeAuthModal;

    /* A delegated capture listener is a second, reliable close path for
       both the Close button and the dark backdrop. */
    document.addEventListener("click", function (event) {
        const closeTarget = event.target.closest(
            ".account-modal-close, .account-modal-backdrop"
        );
    ...
    window.TuklassNotes = (function () {
        const catalog = [
            {catalogId:"algebra", subject:"Mathematics", title:"Study Notes", description:"Algebra study notes and formulas."},
