...
## School + Class account model

This build uses school/class membership stored directly on each Tuklass account.
New students choose a school and section during signup. Calendar and Reminders read the saved Class ID automatically.

Admin routes:
- `admin-classes.html` — add schools/sections and view class rosters
- `admin-reminders.html` — send class-wide reminders by Class ID

Deploy the matching backend update **before** this frontend build.

## V5 assets
The navigation now expects these existing image assets in the repository's `images/` folder:
- `Logo1.png`
- `searchA.png`
- `searchB.png`
