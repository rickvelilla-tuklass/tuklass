...
## V8.9 subscription, caching, and UX behavior

- New accounts choose Monthly (₱150/month with a 7-day free trial) or Yearly (₱1,300/year, saving ₱500 versus 12 monthly payments).
- This build does not process payments online. Payment buttons open the configured Tuklass Facebook contact page.
- Yearly accounts remain `pending_payment` until manually activated; expired/inactive accounts are blocked by both route UI and protected backend actions.
- Notes, flashcard catalogs, decks, and collections use local caches with background refreshes where feasible.
- Collection covers, profile-photo cropping, flashcard retry/results flow, subject grouping, and loading feedback are enabled.
- Run `upgradeTuklassV89()` once after deploying the V8.9 backend.
