...
async function classChanged(){const id=cls();schedule=[];tests=[];render();if(!id)return;msg("Loading calendar...");try{const o=await window.TuklassAdminAuth.post({action:"adminCalendarData",classId:id});if(!o.success)throw new Error(o.error||"Could not load calendar.");schedule=Array.isArray(o.schedule)?o.schedule:[];tests=Array.isArray(o.tests)?o.tests:[];render();msg("");}catch(e){msg(e.message||"Could not load calendar.",true);}}
function v(id){const x=document.getElementById(id);return x?String(x.value||"").trim():"";}
async function post(o){msg("Saving...");const clean={...o};delete clean.email;const j=await window.TuklassAdminAuth.post(clean);if(!j.success){msg(j.error||"Could not save.",true);throw new Error(j.error||"Could not save.");}msg("Saved.",false,true);return j;}
...
(function () {
    ...
    const ROUTE_TEMPLATES = {
        ...
        adminCalendar: `
            <div class="tuklass-route route-admin-calendar">
                ...
                <div class="admin-route-links"><a href="admin.html" class="admin-tab-link">Admin Home</a><a href="admin-classes.html" class="admin-tab-link">Schools & Classes</a><a href="admin-calendar.html" class="admin-tab-link active">Class Calendar</a><a href="admin-reminders.html" class="admin-tab-link">Class Reminders</a></div>
    ...
    function syncShell(user) {
        ...
        /*
         * Admin is a dedicated sidebar destination in V8.2. The link is only
