...
function renderServerMessage(
     ...
     * timestamp can still be revealed by clicking the bubble.
     */
    const showTime =
        !firstInDay &&
        shouldShowTime(
    ...
    return `
        <div
            class="tk-chat-row ${mine ? "is-mine" : "is-theirs"}"
...
function renderPendingMessage(
     ...
     * bubble immediately becomes the normal black sent bubble.
     * We keep it around only until the refreshed server copy
     * arrives, avoiding the old grey-until-refresh behavior.
     */
    return `
        <div
            class="tk-chat-row is-mine ${sending ? "is-sending" : ""} ${failed ? "is-failed" : ""}"
