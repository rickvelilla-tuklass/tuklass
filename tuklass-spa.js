    ...
    if (
        ...
        <div class="registration-card">
            ...
            <p
                ...
                "
            >
                Your Tuklass account does not have a section connected yet.
                If your section was not listed when you created your account,
                you can keep using Tuklass and connect it after an admin adds it.
            </p>

            <a
                href="index.html?completeSection=1"
...
/* =========================================================
   ADMIN NOTES CATALOG ROUTE MODULE — V8.7
========================================================= */
...
window.TuklassAdminCatalog = (function () {
            ...
            return `
                <article class="admin-catalog-row ${note.published ? "" : "is-draft"}">
                    ...
                    <div class="admin-catalog-row-actions">
                        ...
                        <button type="button" onclick="generateAdminCatalogFlashcards('${escapeHtml(note.catalogId)}')">${deck ? "Regenerate cards" : "Generate cards"}</button>
                        ${deck ? `<button type="button" class="danger" onclick="deleteAdminCatalogFlashcards('${escapeHtml(note.catalogId)}')">Remove cards</button>` : ""}
    ...
    function newNote() {
        ...
        const autoFlashcards = document.getElementById("adminCatalogAutoFlashcards");
        if (autoFlashcards) autoFlashcards.checked = true;
    ...
    async function save() {
        ...
        try {
            ...
            const autoFlashcards = document.getElementById("adminCatalogAutoFlashcards");
            const shouldGenerate = !!(aiConfigured && published && published.checked && autoFlashcards && autoFlashcards.checked && savedId);

            if (shouldGenerate) {
                setStatus("Article saved. Generating flashcards with " + aiModel + "...");
                await generateFlashcards(savedId, true);
            }
            else {
                await load();
                if (savedId) edit(savedId);
                setStatus("Article saved.", false, true);
            }
        }
        catch (error) {
    ...
    function cleanup() {
        ["newAdminCatalogNote","editAdminCatalogNote","deleteAdminCatalogNote","saveAdminCatalogNote","generateAdminCatalogFlashcards","generateAdminCatalogFlashcardsFromEditor","deleteAdminCatalogFlashcards","adminCatalogImageSelected","removeAdminCatalogImage"].forEach(function (name) {
...
async function loadCatalog() {
    ...
    if (grid) grid.innerHTML = '<div class="notes-loading">Loading the Tuklass catalog...</div>';

    try {
        const response = await fetch(
            APPS_SCRIPT_URL + "?action=notesCatalog&email=" + encodeURIComponent(currentUser.email),
            {cache:"no-store"}
        );
        ...
        if (grid) grid.innerHTML = '<div class="notes-empty-card error">' + escapeHtml(error.message || "Catalog could not be loaded.") + '</div>';
    }
}
...
function renderCatalog() {
    ...
    const visible = catalogNotes.filter(function (note) {
        ...
        return (note.subject + " " + note.title + " " + note.summary + " " + note.content)
            .toLowerCase().includes(catalogQuery);
    });
    ...
    grid.innerHTML = Object.keys(grouped).sort().map(function (subject) {
        return `
            <section class="wiki-catalog-subject">
...
async function loadCollections() {
    ...
    if (grid) grid.innerHTML = '<div class="notes-loading">Loading collections...</div>';

    try {
        const response = await fetch(
            APPS_SCRIPT_URL + "?action=noteCollections&email=" + encodeURIComponent(currentUser.email),
            {cache:"no-store"}
        );
        const result = await response.json();
        ...
        if (grid) grid.innerHTML = '<div class="notes-empty-card error">' + escapeHtml(error.message || "Collections could not be loaded.") + '</div>';
    }
}
...
/* =========================================================
   FLASHCARDS + FLASHCARD COLLECTIONS ROUTE MODULE — V8.7
========================================================= */
...
async function init() {
    ...
    const search = document.getElementById("flashcardCatalogSearch");
    if (search) {
        search.addEventListener("input", function () {
            deckQuery = search.value.trim().toLowerCase();
            renderDeckCatalog();
        });
    }

    setTab("catalog");
    await Promise.all([loadDecks(), loadCollections()]);

    try {
        const requested = new URL(window.location.href).searchParams.get("deck");
        if (requested && decks.some(deck => deck.deckId === requested)) {
            await openDeck(requested, false);
        }
    } catch {}
}

function setTab(tab) {
    const catalogPanel = document.getElementById("flashcardCatalogPanel");
    const collectionsPanel = document.getElementById("flashcardCollectionsPanel");

    document.querySelectorAll(".flashcards-tab-button").forEach(function (button) {
        button.classList.toggle("active", button.dataset.flashcardsTab === tab);
    });

    if (catalogPanel) catalogPanel.hidden = tab !== "catalog";
    if (collectionsPanel) collectionsPanel.hidden = tab !== "collections";

    if (tab === "catalog") closeDeck(false);
    if (tab === "collections") loadCollections();
}

async function loadDecks() {
    const grid = document.getElementById("flashcardCatalogGrid");
    if (grid) grid.innerHTML = '<div class="notes-loading">Loading flashcards...</div>';

    try {
        const response = await fetch(
            APPS_SCRIPT_URL + "?action=flashcardDecks&email=" + encodeURIComponent(currentUser.email),
            {cache:"no-store"}
        );
        const result = await response.json();
        if (!result.success) throw new Error(result.error || "Flashcards could not be loaded.");
        decks = Array.isArray(result.decks) ? result.decks : [];
        renderDeckCatalog();
    }
    catch (error) {
        if (grid) grid.innerHTML = '<div class="notes-empty-card error">' + escapeHtml(error.message || "Flashcards could not be loaded.") + '</div>';
    }
}

function renderDeckCatalog() {
    const grid = document.getElementById("flashcardCatalogGrid");
    const count = document.getElementById("flashcardCatalogCount");
    if (!grid) return;

    const visible = decks.filter(function (deck) {
        if (!deckQuery) return true;
        return (deck.subject + " " + deck.title + " " + deck.description)
            .toLowerCase().includes(deckQuery);
    });

    if (count) count.textContent = visible.length + (visible.length === 1 ? " deck" : " decks");

    if (!visible.length) {
        grid.innerHTML = `
            <div class="notes-empty-card">
                <strong>${decks.length ? "No decks found" : "No flashcards yet"}</strong>
                <span>${decks.length ? "Try another search." : "Flashcards generated from Notes Catalog articles will appear here."}</span>
            </div>`;
        return;
    }
    ...
    visible.forEach(function (deck) {
        ...
        (grouped[subject] || (grouped[subject] = [])).push(deck);
    });

    grid.innerHTML = Object.keys(grouped).sort().map(function (subject) {
        return `
            <section class="wiki-catalog-subject flashcard-catalog-subject">
                <h3>${escapeHtml(subject)}</h3>
                <div class="wiki-catalog-list">
                    ${grouped[subject].map(function (deck) {
                        const cards = Number(deck.cardCount || 0);
                        return `
                            <button type="button" class="wiki-catalog-row flashcard-deck-row" onclick="openFlashcardDeck('${escapeJs(deck.deckId)}')">
                                <span class="wiki-catalog-row-main">
                                    <strong>${escapeHtml(deck.title)}</strong>
                                    <small>${escapeHtml(deck.description || "Study this deck")}</small>
                                    <em class="catalog-flashcard-badge">${cards} ${cards === 1 ? "card" : "cards"}</em>
                                </span>
                                <span class="wiki-catalog-arrow">›</span>
                            </button>`;
                    }).join("")}
...
async function openDeck(deckId, updateUrl) {
    const browser = document.getElementById("flashcardCatalogBrowser");
    const study = document.getElementById("flashcardStudyView");
    if (!study) return;

    if (browser) browser.hidden = true;
    study.hidden = false;
    study.innerHTML = '<div class="notes-loading">Opening flashcards...</div>';

    try {
        const response = await fetch(
            APPS_SCRIPT_URL + "?action=flashcardDeck&email=" + encodeURIComponent(currentUser.email) + "&deckId=" + encodeURIComponent(deckId),
            {cache:"no-store"}
        );
        const result = await response.json();
        if (!result.success) throw new Error(result.error || "Flashcard deck could not be opened.");

        activeDeck = result.deck || null;
        activeCards = Array.isArray(result.cards) ? result.cards : [];
        activeIndex = 0;
        studyMode = "classic";
        answerVisible = false;
        ...
        if (updateUrl !== false && activeDeck) {
            const url = new URL(window.location.href);
            url.searchParams.set("deck", activeDeck.deckId);
            history.replaceState(history.state, "", url.pathname + url.search);
        }
    }
    catch (error) {
        study.innerHTML = `
            <button type="button" class="wiki-back-button" onclick="closeFlashcardDeck()">← Back to flashcards</button>
            <div class="notes-empty-card error">${escapeHtml(error.message || "Flashcard deck could not be opened.")}</div>`;
    }
}

function renderStudyView() {
    const study = document.getElementById("flashcardStudyView");
    if (!study || !activeDeck) return;

    const count = activeCards.length;
    const card = count ? activeCards[Math.max(0, Math.min(activeIndex, count - 1))] : null;

    study.innerHTML = `
        <button type="button" class="wiki-back-button" onclick="closeFlashcardDeck()">← Back to flashcards</button>
        <header class="flashcard-study-header">
            <div>
                <span class="wiki-eyebrow">${escapeHtml(activeDeck.subject || "Tuklass Flashcards")}</span>
                <h1>${escapeHtml(activeDeck.title || "Flashcards")}</h1>
                <p>${escapeHtml(activeDeck.description || "Study this Tuklass deck.")}</p>
            </div>
            <button type="button" class="button secondary" onclick="chooseFlashcardCollection('${escapeJs(activeDeck.deckId)}')">Add to collection</button>
        </header>

        <div class="flashcard-study-toolbar">
            <div class="flashcard-mode-switch" role="group" aria-label="Flashcard answer mode">
                <button type="button" class="${studyMode === "classic" ? "active" : ""}" onclick="setFlashcardMode('classic')">Classic</button>
                <button type="button" class="${studyMode === "multiple" ? "active" : ""}" onclick="setFlashcardMode('multiple')">A/B/C/D</button>
            </div>
            <span class="flashcard-progress">${count ? (activeIndex + 1) + " / " + count : "0 cards"}</span>
        </div>

        ${card ? renderStudyCard(card) : `
            <div class="notes-empty-card">
                <strong>This deck is empty</strong>
                <span>An administrator can regenerate it from the source note.</span>
            </div>`}

        ${card ? `
            <div class="flashcard-study-nav">
                <button type="button" class="button secondary" onclick="moveFlashcard(-1)" ${activeIndex <= 0 ? "disabled" : ""}>Previous</button>
                <button type="button" class="button primary" onclick="moveFlashcard(1)" ${activeIndex >= count - 1 ? "disabled" : ""}>Next</button>
            </div>` : ""}
    `;
}
...
function renderStudyCard(card) {
    ...
    return `
        <section class="flashcard-stage classic-mode">
            <div class="flashcard-section-label">${escapeHtml(card.section || "Study card")}</div>
            <div class="flashcard-face flashcard-question-face">
                <span>Question</span>
            ...
            ` : `
                <button type="button" class="spa-black-button flashcard-reveal-button" onclick="revealFlashcardAnswer()">Show answer</button>
            `}
        </section>`;
}

function buildChoiceOrder(card) {
    const raw = [card.answer].concat(Array.isArray(card.distractors) ? card.distractors : [])
        .filter(Boolean)
        .slice(0, 4);
    ...
    const seedText = String(card.cardId || card.question || "tuklass");
    let seed = 0;
    ...
    return unique
        .map(function (value, index) {
            const key = (((seed ^ ((index + 1) * 2654435761)) >>> 0) % 1000000);
...
function renderMultipleChoiceCard(card) {
    currentChoices = buildChoiceOrder(card);

    if (currentChoices.length < 4) {
        return `
            <section class="flashcard-stage multiple-mode">
                <div class="flashcard-section-label">${escapeHtml(card.section || "Study card")}</div>
                <div class="flashcard-face flashcard-question-face">
                    <span>Question</span>
                    <h2>${escapeHtml(card.question)}</h2>
                ...
                <div class="flashcard-answer-reveal">
                    <span>Multiple choice unavailable for this card</span>
                    <p>${escapeHtml(card.answer)}</p>
                </div>
            </section>`;
    }
    ...
    const answered = selectedChoiceIndex >= 0;
    ...
    return `
        <section class="flashcard-stage multiple-mode">
            ...
            <div class="flashcard-choices">
                ${currentChoices.map(function (choice, index) {
                    let state = "";
                    if (answered) {
                        if (choice === card.answer) state = " correct";
                        else if (index === selectedChoiceIndex) state = " incorrect";
                    }
                    return `
                        <button type="button" class="flashcard-choice${state}" onclick="answerFlashcardChoice(${index})" ${answered ? "disabled" : ""}>
            ...
            ${answered ? `
                <div class="flashcard-choice-feedback ${currentChoices[selectedChoiceIndex] === card.answer ? "correct" : "incorrect"}">
                    <strong>${currentChoices[selectedChoiceIndex] === card.answer ? "Correct" : "Not quite"}</strong>
...
function setMode(mode) {
    studyMode = mode === "multiple" ? "multiple" : "classic";
    answerVisible = false;
    selectedChoiceIndex = -1;
    currentChoices = [];
    renderStudyView();
...
function answerChoice(index) {
    ...
    selectedChoiceIndex = Number(index);
    renderStudyView();
}

function moveCard(direction) {
    if (!activeCards.length) return;
    activeIndex = Math.max(0, Math.min(activeCards.length - 1, activeIndex + Number(direction || 0)));
    answerVisible = false;
    selectedChoiceIndex = -1;
    currentChoices = [];
    renderStudyView();
...
function closeDeck(updateUrl) {
    ...
    const study = document.getElementById("flashcardStudyView");
    if (browser) browser.hidden = false;
    if (study) { study.hidden = true; study.innerHTML = ""; }
    activeDeck = null;
    activeCards = [];
    activeIndex = 0;
    answerVisible = false;
...
async function loadCollections() {
    if (!currentUser) return;
    const grid = document.getElementById("flashcardCollectionGrid");
    if (grid) grid.innerHTML = '<div class="notes-loading">Loading collections...</div>';

    try {
        const response = await fetch(
            APPS_SCRIPT_URL + "?action=flashcardCollections&email=" + encodeURIComponent(currentUser.email),
            {cache:"no-store"}
        );
        const result = await response.json();
        if (!result.success) throw new Error(result.error || "Collections could not be loaded.");
...
function renderCollections() {
    ...
    if (!collections.length) {
        grid.innerHTML = `
            <div class="notes-empty-card">
                <strong>No flashcard collections yet</strong>
                <span>Create a collection for a unit, exam, or subject.</span>
            </div>`;
        return;
    }
    ...
    grid.innerHTML = collections.map(function (collection) {
        const count = Number(collection.itemCount || 0);
        return `
            <article class="study-collection-card flashcard-collection-card">
                <button type="button" class="study-collection-open" onclick="openFlashcardCollection('${escapeJs(collection.collectionId)}')">
                    <span class="collection-folder-mark"></span>
                    <span>
                        <strong>${escapeHtml(collection.title)}</strong>
                        <small>${count} ${count === 1 ? "deck" : "decks"}</small>
                    </span>
                </button>
                <button type="button" class="collection-more-button" onclick="deleteFlashcardCollection('${escapeJs(collection.collectionId)}')">Delete</button>
            </article>`;
    }).join("");
}

async function createCollection() {
    const title = value("newFlashcardCollectionTitle");
    const description = value("newFlashcardCollectionDescription");
    if (!title) { setMessage("Give the collection a name.", true); return; }

    try {
        const result = await post({action:"createFlashcardCollection", email:currentUser.email, title:title, description:description});
        const titleEl = document.getElementById("newFlashcardCollectionTitle");
        const descriptionEl = document.getElementById("newFlashcardCollectionDescription");
        if (titleEl) titleEl.value = "";
        if (descriptionEl) descriptionEl.value = "";
        await loadCollections();
        if (result.collectionId) await openCollection(result.collectionId);
        setMessage("Collection created.", false);
    }
    catch (error) { setMessage(error.message || "Collection could not be created.", true); }
}
...
async function deleteCollection(collectionId) {
    ...
    try {
        await post({action:"deleteFlashcardCollection", email:currentUser.email, collectionId:collectionId});
        if (activeCollection && activeCollection.collectionId === collectionId) closeCollection();
        await loadCollections();
        setMessage("Collection deleted.", false);
    }
    catch (error) { setMessage(error.message || "Collection could not be deleted.", true); }
}

async function openCollection(collectionId) {
    const workspace = document.getElementById("flashcardCollectionWorkspace");
    if (workspace) workspace.hidden = false;

    try {
        const response = await fetch(
            APPS_SCRIPT_URL + "?action=flashcardCollection&email=" + encodeURIComponent(currentUser.email) + "&collectionId=" + encodeURIComponent(collectionId),
            {cache:"no-store"}
        );
        const result = await response.json();
        if (!result.success) throw new Error(result.error || "Collection could not be opened.");
        activeCollection = result.collection || null;
        if (activeCollection) activeCollection.items = Array.isArray(result.items) ? result.items : [];
        renderCollectionWorkspace();
        if (workspace) workspace.scrollIntoView({behavior:"smooth", block:"start"});
    }
    catch (error) { setMessage(error.message || "Collection could not be opened.", true); }
}
...
function renderCollectionWorkspace() {
    ...
    workspace.hidden = false;
    const title = document.getElementById("flashcardCollectionWorkspaceTitle");
    const description = document.getElementById("flashcardCollectionWorkspaceDescription");
    const list = document.getElementById("flashcardCollectionDecks");

    if (title) title.textContent = activeCollection.title || "Flashcard collection";
    if (description) description.textContent = activeCollection.description || "A personal Tuklass flashcard collection.";

    const items = Array.isArray(activeCollection.items) ? activeCollection.items : [];
    if (!list) return;

    if (!items.length) {
        list.innerHTML = '<div class="notes-empty-card"><strong>This collection is empty</strong><span>Open a flashcard deck and add it to this collection.</span></div>';
        return;
    }
    ...
    list.innerHTML = items.map(function (item) {
        ...
        return `
            <article class="flashcard-collection-deck-row">
                <button type="button" onclick="studyDeckFromCollection('${escapeJs(deck.deckId || "")}')">
                    <span>${escapeHtml(deck.subject || "General")}</span>
                    <strong>${escapeHtml(deck.title || "Flashcards")}</strong>
                    <small>${Number(deck.cardCount || 0)} cards</small>
                </button>
                <button type="button" class="danger" onclick="removeFlashcardCollectionItem('${escapeJs(item.itemId || "")}')">Remove</button>
            </article>`;
    }).join("");
}

function studyDeckFromCollection(deckId) {
    setTab("catalog");
    openDeck(deckId, true);
}
...
async function removeCollectionItem(itemId) {
    ...
    try {
        await post({
            action:"removeDeckFromFlashcardCollection",
            email:currentUser.email,
    ...
    catch (error) { setMessage(error.message || "Deck could not be removed.", true); }
}

function chooseCollection(deckId) {
    selectedDeckId = deckId;
    const modal = document.getElementById("flashcardCollectionPickerModal");
    const select = document.getElementById("flashcardCollectionSelect");
    if (!modal || !select) return;

    if (!collections.length) {
        window.alert("Create a flashcard collection first.");
        setTab("collections");
        return;
    }

    select.innerHTML = collections.map(function (collection) {
        return '<option value="' + escapeAttribute(collection.collectionId) + '">' + escapeHtml(collection.title) + '</option>';
    }).join("");
    modal.hidden = false;
}
...
async function confirmAdd() {
    const select = document.getElementById("flashcardCollectionSelect");
    const collectionId = select ? select.value : "";
    if (!selectedDeckId || !collectionId) return;

    try {
        const result = await post({
            action:"addDeckToFlashcardCollection",
            email:currentUser.email,
        ...
        closePicker();
        await loadCollections();
        setMessage(result.alreadyAdded ? "That deck is already in the collection." : "Deck added to collection.", false);
    }
    catch (error) { setMessage(error.message || "Deck could not be added.", true); }
}
...
function setMessage(text, isError) {
    const el = document.getElementById("flashcardCollectionMessage");
    if (!el) return;
    el.textContent = text || "";
    el.className = "collection-message" + (isError ? " error" : "");
}
...
function registerGlobals() {
    window.setFlashcardsTab = setTab;
    window.openFlashcardDeck = openDeck;
    window.closeFlashcardDeck = closeDeck;
    window.setFlashcardMode = setMode;
    window.revealFlashcardAnswer = revealAnswer;
    window.answerFlashcardChoice = answerChoice;
    window.moveFlashcard = moveCard;
    window.createFlashcardCollection = createCollection;
    window.deleteFlashcardCollection = deleteCollection;
    window.openFlashcardCollection = openCollection;
    window.closeFlashcardCollection = closeCollection;
    window.removeFlashcardCollectionItem = removeCollectionItem;
    window.chooseFlashcardCollection = chooseCollection;
    window.closeFlashcardCollectionPicker = closePicker;
    window.confirmFlashcardCollectionAdd = confirmAdd;
    window.studyDeckFromCollection = studyDeckFromCollection;
}

function cleanup() {
    [
        "setFlashcardsTab","openFlashcardDeck","closeFlashcardDeck","setFlashcardMode",
        "revealFlashcardAnswer","answerFlashcardChoice","moveFlashcard","createFlashcardCollection",
        "deleteFlashcardCollection","openFlashcardCollection","closeFlashcardCollection",
        "removeFlashcardCollectionItem","chooseFlashcardCollection","closeFlashcardCollectionPicker",
        "confirmFlashcardCollectionAdd","studyDeckFromCollection"
    ].forEach(function (name) {
        try { delete window[name]; } catch {}
    });
            ...
            <div class="tuklass-route route-admin-catalog">
                ...
                <div class="admin-catalog-layout">
                    <section id="adminCatalogEditor" class="admin-catalog-editor">
                        ...
                        <section class="admin-ai-flashcards-card">
                            <div class="admin-ai-flashcards-head">
                                <div>
                                    <span class="notes-section-kicker">AI study deck</span>
                                    <h3>Generate Flashcards</h3>
                                </div>
                                <label class="admin-flashcard-count">
                                    <span>Cards</span>
                                    <input id="adminCatalogFlashcardCount" type="number" min="4" max="40" value="15">
                                </label>
                            </div>
                            <p>Tuklass sends this saved article to the configured AI model and stores the generated cards. Every card includes three distractors so students can switch between classic cards and A/B/C/D mode.</p>
                            <div id="adminCatalogFlashcardStatus" class="admin-ai-status"><strong>Loading AI status...</strong></div>
                            <label class="admin-catalog-publish-toggle admin-ai-auto-toggle"><input id="adminCatalogAutoFlashcards" type="checkbox" checked><span>Automatically generate or refresh flashcards after I save a published article</span></label>
                            <button id="adminCatalogGenerateFlashcards" type="button" class="button secondary route-full-button" onclick="generateAdminCatalogFlashcardsFromEditor()" disabled>Generate Flashcards</button>
                        </section>
